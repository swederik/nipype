...
<<<<<<< HEAD
	...
	MRconvert_tensor = MRconvert_fa.clone(name='MRconvert_tensor')
	MRconvert_wmmask = MRconvert_fa.clone(name='MRconvert_wmmask')
	...
	applyxfm_trackprobimage = pe.Node(interface=fsl.ApplyXfm(), name = 'applyxfm_trackprobimage')
	applyxfm_trackprobimage.inputs.apply_xfm = True

	applyxfm_fa = applyxfm_trackprobimage.clone(name='applyxfm_fa')
	applyxfm_tensor = applyxfm_trackprobimage.clone(name='applyxfm_tensor')
	applyxfm_wmmask = applyxfm_trackprobimage.clone(name='applyxfm_wmmask')
	...
	phrases = ['frontalpole', 'occipital']
	phrases.append('occipital')
	...
	mapping.connect([(dwi2tensor, MRconvert_tensor,[("tensor","in_file")])])
	...
	mapping.connect([(gen_WM_mask, MRconvert_wmmask,[("WMprobabilitymap","in_file")])])
	mapping.connect([(MRconvert_wmmask, applyxfm_wmmask,[("converted","in_file")])])
	mapping.connect([(coregister, applyxfm_wmmask,[('out_matrix_file','in_matrix_file')])])
	mapping.connect([(mri_convert_Brain, applyxfm_wmmask,[('out_file','reference')])])
	...
	mapping.connect([(MRconvert_tracks2prob, applyxfm_trackprobimage,[("converted","in_file")])])
	mapping.connect([(coregister, applyxfm_trackprobimage,[('out_matrix_file','in_matrix_file')])])
	mapping.connect([(mri_convert_Brain, applyxfm_trackprobimage,[('out_file','reference')])])

	mapping.connect([(MRconvert_tensor, applyxfm_tensor,[("converted","in_file")])])
	mapping.connect([(coregister, applyxfm_tensor,[('out_matrix_file','in_matrix_file')])])
	mapping.connect([(mri_convert_Brain, applyxfm_tensor,[('out_file','reference')])])

	mapping.connect([(MRconvert_fa, applyxfm_fa,[("converted","in_file")])])
	mapping.connect([(coregister, applyxfm_fa,[('out_matrix_file','in_matrix_file')])])
	mapping.connect([(mri_convert_Brain, applyxfm_fa,[('out_file','reference')])])
	...
	mapping.connect([(creatematrix, node_removal,[("intersection_matrix_file","in_files")])])
	mapping.connect([(node_removal, networkx,[("out_files","inputnode.network_file")])])
	#mapping.connect([(creatematrix, CFFConverter,[("matrix_files","gpickled_networks")])])
	mapping.connect([(node_removal, CFFConverter,[("out_files","gpickled_networks")])])
	...
	connectivity.connect([(mapping, outputnode, [("tck2trk.out_file", "tracts"),
		...
		("node_removal.out_files", "networks"),
		...
		("applyxfm_trackprobimage.out_file", "tracks2prob")])
		])
