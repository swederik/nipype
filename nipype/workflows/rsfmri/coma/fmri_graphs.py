...
import nipype.interfaces.ComaRestLib as crl
...
def create_fmri_graphs(name="functional", with_simple_timecourse_correlation=False):
    ...
    denoised_image = pe.Node(interface=crl.CreateDenoisedImage(), name='denoised_image')
    denoised_image.inputs.coma_rest_lib_path = coma_rest_lib_path
    matching_classification = pe.Node(interface=crl.MatchingClassification(), name='matching_classification')
    matching_classification.inputs.coma_rest_lib_path = coma_rest_lib_path
    compute_fingerprints = pe.MapNode(interface=crl.ComputeFingerprint(), name='compute_fingerprints', iterfield=['in_file', 'component_index'])
