...
class CreateMatrixOutputSpec(TraitedSpec):
    ...
    intersection_matrix_file = File(desc='NetworkX graph describing the connectivity')
    matrix_files = OutputMultiPath(File(desc='All of the gpickled network files output by this interface'))
    matlab_matrix_files = OutputMultiPath(File(desc='All of the MATLAB .mat files output by this interface'))
    matrix_mat_file = File(desc='Matlab matrix describing the connectivity')
    intersection_matrix_mat_file = File(desc='Matlab matrix describing the mean fiber lengths between each node.')
    mean_fiber_length_matrix_mat_file = File(desc='Matlab matrix describing the mean fiber lengths between each node.')
    median_fiber_length_matrix_mat_file = File(desc='Matlab matrix describing the median fiber lengths between each node.')
...
def voxel_regions_from_image(roiData, weight_threshold, above_threshold):
	if above_threshold:
		voxels = roiData > weight_threshold
	else:
		voxels = roiData < weight_threshold
	return voxels


def fibers_from_voxel_regions(streamlines, voxels, roiVoxelSize, consider_region_intersections):
	n_fib = len(streamlines)
	final_fiber_ids = []
	final_fiber_ids_intersections = []
	pc = 0
	for i, fiber in enumerate(streamlines):
		...
		if pcN > pc and pcN % 1 == 0:
			pc = pcN
			print '%4.0f%%' % (pc)
		rois_crossed_bool, endpoint_bool = crossed(fiber[0], voxels, roiVoxelSize)
		if rois_crossed_bool:
			final_fiber_ids_intersections.append(i)
			if endpoint_bool:
				final_fiber_ids.append(i)
	print 'Total fibers that started/ended in an ROI: %4.0f%% or %4.0f%%' % (len(final_fiber_ids), float(len(final_fiber_ids))/n_fib)	
	print 'Total fibers that passed through the ROI: %4.0f%% or %4.0f%%' % (len(final_fiber_ids_intersections), float(len(final_fiber_ids_intersections))/n_fib)
	return final_fiber_ids, final_fiber_ids_intersections

def crossed(pointsmm, roiData, voxelSize):
	n_points = len(pointsmm)
	rois_crossed = []
	crossed = False
	endpoint = False
	...
	for j in xrange(0, n_points):
		...
		if not roiData[x, y, z] == 0:
			...
			if j==0 or j==n_points:
				endpoint = True
				break
	return crossed, endpoint

class TractsInImageInputSpec(TraitedSpec):
    tract_file = File(exists=True, mandatory=True, desc='Trackvis tract file')
    image_file = File(exists=True, mandatory=True, desc="Subject's connectivity graph")
    consider_region_intersections = traits.Bool(True, usedefault=True, desc='Counts all of the fiber-region traversals in the connectivity matrix (requires significantly more computational time)')
    above_threshold = traits.Bool(True, usedefault=True, desc='Only connectivity values greater than or equal to the threshold will be used')
    weight_threshold = traits.Float(0, usedefault=True, desc='Connectivity weight threshold (default 0.05, for NBS graph analysis)')
    out_tract_name = File('image_filtered.trk', usedefault=True, desc='Name for output tract file')

class TractsInImageOutputSpec(TraitedSpec):
    filtered_tractography = File(desc='TrackVis file containing only those fibers originate in one and terminate in another region')
    filtered_tractography_by_intersections = File(desc='TrackVis file containing all fibers which connect two regions')
    filtered_tractographies = OutputMultiPath(File(desc='TrackVis file containing only those fibers originate in one and terminate in another region'))

class TractsInImage(BaseInterface):
    """
    Identifies tracts passing through regions of an image:

    Given a Nifti image, an ROI file, a set of Tracts,
    and a weight threshold this interface will return that
    tracts that traverse the sub/suprathreshold connections
    in the graph.
    ...
    >>> import nipype.interfaces.cmtk as cmtk
    >>> trkbtwn = cmtk.TractsInImage()
    >>> trkbtwn.inputs.tract_file = 'fibers.trk'
    >>> trkbtwn.inputs.image_file = 'activation.nii'
    >>> trkbtwn.run()                 # doctest: +SKIP
    """

    input_spec = TractsInImageInputSpec
    output_spec = TractsInImageOutputSpec

    def _run_interface(self, runtime):
        iflogger.info('Identifying fibers between given regions...')

        iflogger.info('Reading image file {ntwk}'.format(ntwk=self.inputs.image_file))
        roi_image = nb.load(self.inputs.image_file)
        roiData = roi_image.get_data()
        roiVoxelSize = roi_image.get_header().get_zooms()
        ...
        voxels = voxel_regions_from_image(roiData, self.inputs.weight_threshold, self.inputs.above_threshold)
        final_fiber_ids, final_fiber_ids_intersections = fibers_from_voxel_regions(fib, voxels, roiVoxelSize, self.inputs.consider_region_intersections)
